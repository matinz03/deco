...
| # | Item | Evidence | Notes |
|---|---|---|---|
| D-1 | **Implemented on `codex/mvp-integration`.** CI builds, vets, and tests Go against live pgvector/Postgres and MinIO services; runs crypto tests and TypeScript checks; and performs a production Next.js build. | `.github/workflows/ci.yml` | Lint remains deliberately omitted until D-9 adds a real ESLint gate. Branch protection still must be enabled in GitHub after merge. |
...
| D-3 | **Implemented on `codex/mvp-integration`.** Deploys are locked, refuse tracked local edits, take a backup, wait for app health, and rebuild the previous commit on failure. | `deploy.sh`, `watch-deploy.sh` | Rollback is application-level; schema changes must remain backward compatible. |
| D-4 | **Implemented on `codex/mvp-integration`.** Timestamped PostgreSQL + MinIO snapshots go to separate S3-compatible storage before deploy and daily through a systemd timer; restore verifies the dump checksum. | `backup.sh`, `restore.sh`, `infra/systemd/deco-backup.*` | Off-host credentials and a successful disposable restore rehearsal remain launch-operator requirements. |
| D-5 | **Implemented on `codex/mvp-integration`.** API and web have application-level healthchecks, and web waits for a healthy API before starting. | `docker-compose.yml`, `/health`, `/api/health` | A deploy-level rollback remains D-3. |
...
| D-9 | **There is no linting in this repo, and `pnpm lint` fails outright.** `apps/web`'s script is `next lint`, removed in Next.js 16 — the CLI parses `lint` as a directory and errors with `Invalid project directory provided`. No `.eslintrc*`/`eslint.config.*` and no `eslint` dependency exists anywhere; `packages/config` advertises an `"./eslint"` export whose target file is absent. Verified by running it. **Rejected non-fix:** aliasing `lint` to `tsc --noEmit` makes it a duplicate of `type-check` while enforcing zero rules — a gate that reports green and checks nothing. Do not do this. | `apps/web/package.json`, `packages/config/package.json:7` | Real setup work: `eslint` + flat config + rule selection for Next 16 / React 19. Until it exists, CI's lint step must be **explicitly omitted with a reference here**, never silently weakened. |
