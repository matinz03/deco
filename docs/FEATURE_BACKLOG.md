...
| # | Item | Evidence | Notes |
|---|---|---|---|
| D-1 | **Implemented on `codex/mvp-integration`.** CI builds, vets, and tests Go against a live pgvector/Postgres schema; runs crypto tests and TypeScript checks; and performs a production Next.js build. | `.github/workflows/ci.yml` | Lint remains deliberately omitted until D-9 adds a real ESLint gate. Branch protection still must be enabled in GitHub after merge. |
